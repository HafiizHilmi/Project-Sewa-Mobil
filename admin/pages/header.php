...
===================================================== -->
<header
  class="flex-shrink-0 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 px-5 lg:px-6 py-3 flex items-center justify-between gap-3 z-10">
  ...
  <div class="flex items-center gap-2 flex-shrink-0">
    <button
      class="relative w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center transition-colors">
      <i class="fa-solid fa-bell text-slate-500 dark:text-slate-400 text-sm"></i>
      <span
        class="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-red-500 border-2 border-white dark:border-slate-800"></span>
    </button>
    ...
    <button x-show="activePage === 'cars'" @click="$dispatch('open-add-car-modal')" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg ...">
    + Tambah Mobil
</button>
  </div>
</header>